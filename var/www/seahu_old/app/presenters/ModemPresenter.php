...
class ModemPresenter extends BasePresenter
{
	...
	protected function modemGetActualSeting()
	{
		...
		$this->template->device = $lines[1];
		$this->template->baud = $lines[2];
		$this->template->dial = $lines[3];
		$this->template->apn = $lines[4];
		$this->template->ip = $lines[5];
		$this->template->netmask = $lines[6];
		$this->template->gateway = $lines[7];
		$this->template->dns = $lines[8];
		$this->template->signal = $lines[9];
	}
