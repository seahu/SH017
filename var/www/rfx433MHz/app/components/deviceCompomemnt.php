...
class deviceControl extends UI\Control
{
	...
	*/
	public function loadConfig()
	{
		...
		$this->cache=$parent->cache; // pouziti cache z predka
	...
	*/
	...
	public function sendRfxCode($code)
	{
		...
		$shell="/usr/bin/sudo /opt/seahu/rfx433MHz/codesend ".$code; // v predkovi vytvorit slolecnou fci na rfx odeslini a prijem
	...
	public function handleSetRFXpartCode($switch,$status)
	{
		...
		$new_code=substr_replace($this->getCode(),$val,$switch,1);
	...
	public function handleStartScanRFXCode() // vytvorit fci nastvi odchytavaci priznak pro funkci serveRFXsignal($signalCode) ktera pote naskenovany cod ulozi
	{
		...
		$this->config->scanCodes=array(); // vynuluj pole drive naskenovanych hodnot
		$this->saveConfig(); 
	}
	...
	public function handleStopScanRFXCode() // vytvorit fci vypne odchytavaci priznak pro funkci serveRFXsignal
	{
		...
		if ($this->config->scanning=="RUN") $this->config->scanCode=$selectCode;
